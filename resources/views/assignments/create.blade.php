<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl font-semibold text-[#333333]">Create Assignment</h2>
        </div>
    </x-slot>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 max-w-2xl">
            <div class="grid grid-cols-2 gap-6">
                <div class="col-span-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md text-sm" placeholder="e.g., Quiz 1 - Addition">
                </div>
                ...
                <div>
                    ...
                    <select name="type" id="type" required class="w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md text-sm">
                        <option value="written_work" {{ old('type') == 'written_work' ? 'selected' : '' }}>Written Work (WW)</option>
                        <option value="performance_task" {{ old('type') == 'performance_task' ? 'selected' : '' }}>Performance Task (PT)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="max_score" id="max_score" value="{{ old('max_score') }}" required step="0.01" min="1" class="w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md text-sm" placeholder="e.g., 50">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="due_date" id="due_date" value="{{ old('due_date') }}" class="w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md text-sm">
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" class="w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md text-sm" placeholder="Brief description of the assignment...">{{ old('description') }}</textarea>
                </div>
            </div>

            <div class="mt-6 flex items-center justify-between">
                <a href="{{ route('assignments.index') }}" class="text-sm text-[#666666] hover:text-[#333333]">Cancel</a>
